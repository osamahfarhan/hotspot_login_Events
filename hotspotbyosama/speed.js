...
function osamaSpeedHiddeAll() {
    osamaHiddeThis((document.querySelectorAll("[data-sp]")));
    osamaHiddeThis((document.querySelectorAll("[data-spi]")));
    osamaHiddeThis((document.querySelectorAll("[data-spsl]")));
...
function osamaSpeedShowSelect(e, n = -1) {
    ...
    myupd=UPD;
    osamaSpeedHiddeAll();
    var selectOptions = document.querySelectorAll("[data-sp]");
    var radioInputs = document.querySelectorAll("[data-spi]");
    var speedLabels = document.querySelectorAll("[data-spsl]");
    ...
    if ("object" == typeof SPN && osamaIsSpeed(SPN)) {
        for (var b in SPN) {
            ...
            try {
                osamaShowElem(document.getElementById('loginspeeds'));
                document.getElementById('show-select').removeAttribute('selected');
                document.getElementById('show-select').style.display = 'none';
                (document.getElementById("loginspeeds")).options.selectedIndex = (NSP + 1);
                osamaSpeedSelect = NSP;
                (document.getElementById("loginspeeds")).value = NSP;
            } catch (error) { };
        ...
        if (document.getElementById('loginspeeds')) {
            try {
                document.getElementById('loginspeeds').style.display = 'none';
                (document.getElementById('loginspeeds')).options.selectedIndex = 0;
                document.getElementById('loginspeeds').value = '100';
            } catch (error) { };
        }        
    }
    ...
    // osamaSpeedRemoveSelect();
    if (document.getElementById("loginspeeds")) {
        var selectOptions = (document.getElementById("loginspeeds")).options;
        try {
            if (osamaSpeedSelect > -1 && osamaSpeedSelect < 19) {
                (document.getElementById("loginspeeds")).options.selectedIndex = (osamaSpeedSelect + 1);
                (document.getElementById("loginspeeds")).value = osamaSpeedSelect;
            }
        } catch (er) { }
        document.getElementById("loginspeeds").addEventListener("change", function () {
            var b = (document.getElementById("loginspeeds")).options.selectedIndex;
        ...
        document.getElementById("loginspeeds").addEventListener("click", function () {
            var b = (document.getElementById("loginspeeds")).options.selectedIndex;
            osamaSpeedSelect = parseInt(selectOptions[b].value);
            if (MyPro) { MyPro.innerText = selectOptions[b].innerText; }
            if (radioInputs&&radioInputs[osamaSpeedSelect]) { radioInputs[osamaSpeedSelect].setAttribute('checked', 'true'); radioInputs[osamaSpeedSelect].setAttribute('aria-checked', 'true');radioInputs[osamaSpeedSelect].checked = true;}            
